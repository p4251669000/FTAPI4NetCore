...
namespace Futu.OpenApi
{
    public interface FTSPI_Trd
    {
        ...
        void OnReply_GetMarginRatio(FTAPI_Conn client, uint nSerialNo, TrdGetMarginRatio.Response rsp); //获取融资融券数据
        void OnReply_GetOrderFee(FTAPI_Conn client, uint nSerialNo, TrdGetOrderFee.Response rsp); //获取订单费用
        void OnReply_GetFlowSummary(FTAPI_Conn client, uint nSerialNo, TrdFlowSummary.Response rsp); //获取资金流水
    ...
    public class FTAPI_Trd : FTAPI_Conn
    {
        ...
        /// <returns>请求的序列号</returns>
        public uint GetMarginRatio(TrdGetMarginRatio.Request req)
        {
            return SendProto((uint)ProtoID.TrdGetMarginRatio, req);
        }
        /// <summary>
        /// 查询指定账号服务器订单的订单费用
        /// </summary>
        /// <param name="req"></param>
        /// <returns>请求的序列号</returns>
        public uint GetOrderFee(TrdGetOrderFee.Request req)
        {
            return SendProto((uint)ProtoID.TrdGetOrderFee, req);
        }
        /// <summary>
        /// 取资金流水
        /// </summary>
        /// <param name="req"></param>
        /// <returns>请求的序列号</returns>
        public uint GetFlowSummary(TrdFlowSummary.Request req)
        {
            return SendProto((uint)ProtoID.TrdGetFlowSummary, req);
        }
        ...
        protected override void OnReply(ReqReplyType replyType, ProtoHeader protoHeader, byte[] data)
        {
            ...
            switch (protoID)
            {
                ...
                case ProtoID.TrdGetMarginRatio://获取融资融券数据
                    {
                        Futu.OpenApi.Pb.TrdGetMarginRatio.Response rsp;
                        if (replyType == ReqReplyType.SvrReply)
                        {
                            try
                            {
                                rsp = Futu.OpenApi.Pb.TrdGetMarginRatio.Response.ParseFrom(data);
                            }
                            catch (InvalidProtocolBufferException)
                            {
                                rsp = Futu.OpenApi.Pb.TrdGetMarginRatio.Response.CreateBuilder().SetRetType((int)Common.RetType.RetType_Invalid).Build();
                            }
                        }
                        else
                        {
                            rsp = Futu.OpenApi.Pb.TrdGetMarginRatio.Response.CreateBuilder().SetRetType((int)replyType).Build();
                        }

                        trdCallback.OnReply_GetMarginRatio(this, serialNo, rsp);
                    }
                    break;
                case ProtoID.TrdGetOrderFee://获取订单费用
                    {
                        Futu.OpenApi.Pb.TrdGetOrderFee.Response rsp;
                        if (replyType == ReqReplyType.SvrReply)
                        {
                            try
                            {
                                rsp = Futu.OpenApi.Pb.TrdGetOrderFee.Response.ParseFrom(data);
                            }
                            catch (InvalidProtocolBufferException)
                            {
                                rsp = Futu.OpenApi.Pb.TrdGetOrderFee.Response.CreateBuilder().SetRetType((int)Common.RetType.RetType_Invalid).Build();
                            }
                        }
                        else
                        {
                            rsp = Futu.OpenApi.Pb.TrdGetOrderFee.Response.CreateBuilder().SetRetType((int)replyType).Build();
                        }

                        trdCallback.OnReply_GetOrderFee(this, serialNo, rsp);
                    }
                    break;
                case ProtoID.TrdGetFlowSummary: //获取资金流水
                    {
                        Futu.OpenApi.Pb.TrdFlowSummary.Response rsp;
                        if (replyType == ReqReplyType.SvrReply)
                        {
                            try
                            {
                                rsp = Futu.OpenApi.Pb.TrdFlowSummary.Response.ParseFrom(data);
                            }
                            catch (InvalidProtocolBufferException)
                            {
                                rsp = Futu.OpenApi.Pb.TrdFlowSummary.Response.CreateBuilder().SetRetType((int)Common.RetType.RetType_Invalid).Build();
                            }
                        }
                        else
                        {
                            rsp = Futu.OpenApi.Pb.TrdFlowSummary.Response.CreateBuilder().SetRetType((int)replyType).Build();
                        }

                        trdCallback.OnReply_GetFlowSummary(this, serialNo, rsp);
                    }
                    break;
            }
        }
