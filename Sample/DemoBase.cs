...
namespace FTAPI4NetSample
{
    ...
    class DemoBase : FTSPI_Conn, FTSPI_Qot, FTSPI_Trd
    {
        ...
        /// 获取第一个指定交易能力和环境的账户id
        /// </summary>
        /// <param name="trdMarket">业务账户支持的交易市场权限，即此账户能交易那些市场, 可拥有多个交易市场权限，目前仅单个</param>
        /// <param name="trdEnv">交易环境</param>
        /// <returns></returns>
        public ulong GetFirstAccId(TrdCommon.TrdMarket trdMarket, TrdCommon.TrdEnv trdEnv)
        {
            TrdGetAccList.Response getAccListRsp = GetAccListSync();
            ulong accID = 0;
            foreach (TrdCommon.TrdAcc acc in getAccListRsp.S2C.AccListList)
            {
                if (acc.TrdEnv == (int)trdEnv && acc.TrdMarketAuthListList[0] == (int)trdMarket)
                {
                    accID = acc.AccID;
                    break;
                }
        ...
        public QotGetSecuritySnapshot.Response GetSecuritySnapshotSync(ICollection<QotCommon.Security> secList)
        {
            ...
            lock (syncEvent)
            {
                ...
                QotGetSecuritySnapshot.Response resp = (QotGetSecuritySnapshot.Response)reqInfo.Rsp;
                if (resp.RetType != (int)Common.RetType.RetType_Succeed)
                {
                    Console.Write("ERROR: GetSecuritySnapshot, retMsg = {0}\n", resp.RetMsg);
                }
                return resp;
            }
        }
        ...
        public QotGetStaticInfo.Response GetStaticInfoSync(QotGetStaticInfo.C2S c2s)
        {
            ...
            lock (syncEvent)
            {
                ...
                Monitor.Wait(syncEvent);
                QotGetStaticInfo.Response resp = (QotGetStaticInfo.Response)reqInfo.Rsp;
                if (resp.RetType != (int)Common.RetType.RetType_Succeed)
                {
                    Console.Write("ERROR: GetStaticInfo, retMsg = {0}\n", resp.RetMsg);
                }
                return resp;
            }
        }
                                ...
                                bool isRegPush)
        {
            ...
            lock (syncEvent)
            {
                ...
                Monitor.Wait(syncEvent);
                QotSub.Response resp = (QotSub.Response)reqInfo.Rsp;
                if (resp.RetType != (int)Common.RetType.RetType_Succeed)
                {
                    Console.Write("ERROR: Sub, retMsg = {0}\n", resp.RetMsg);
                }
                return resp;
            }
        }
        ...
        public QotGetOrderBook.Response GetOrderBookSync(QotCommon.Security sec, int num)
        {
            ...
            lock (syncEvent)
            {
                ...
                Monitor.Wait(syncEvent);
                QotGetOrderBook.Response resp = (QotGetOrderBook.Response)reqInfo.Rsp;
                if (resp.RetType != (int)Common.RetType.RetType_Succeed)
                {
                    Console.Write("ERROR: GetOrderBook, retMsg = {0}\n", resp.RetMsg);
                }
                return resp;
            }
        }
                                                          ...
                                                          bool extendedTime)
        {
            ...
            lock (syncEvent)
            {
                ...
                Monitor.Wait(syncEvent);
                QotRequestHistoryKL.Response resp = (QotRequestHistoryKL.Response)reqInfo.Rsp;
                if (resp.RetType != (int)Common.RetType.RetType_Succeed)
                {

                    Console.WriteLine("ERROR: RequestHistoryKL, retMsg = {0}", resp.RetMsg);
                }
                return resp;
            }
        ...
        public TrdPlaceOrder.Response PlaceOrderSync(TrdPlaceOrder.C2S c2s)
        {
            ...
            lock (syncEvent)
            {
                ...
                Monitor.Wait(syncEvent);
                TrdPlaceOrder.Response resp = (TrdPlaceOrder.Response)reqInfo.Rsp;
                if (resp.RetType != (int)Common.RetType.RetType_Succeed)
                {
                    Console.WriteLine("ERROR: PlaceOrder, retMsg = {0}", resp.RetMsg);
                }
                return resp;
            }
        }

        public TrdModifyOrder.Response ModifyOrderSync(TrdModifyOrder.C2S c2s)
        {
            ReqInfo reqInfo = null;
            Object syncEvent = new Object();
            ...
            lock (syncEvent)
            {
                ...
                Monitor.Wait(syncEvent);
                TrdModifyOrder.Response resp = (TrdModifyOrder.Response)reqInfo.Rsp;
                if (resp.RetType != (int)Common.RetType.RetType_Succeed)
                {
                    Console.WriteLine("ERROR: ModifyOrder, retMsg = {0}", resp.RetMsg);
                }
                return resp;
            }
        }

        public TrdUnlockTrade.Response UnlockTradeSync(String pwdMD5, TrdCommon.SecurityFirm securityFirm, bool isUnlock)
        {
            ReqInfo reqInfo = null;
            Object syncEvent = new Object();
            ...
            lock (syncEvent)
            {
                lock (trdLock)
                {
                    ...
                    TrdUnlockTrade.C2S c2s = TrdUnlockTrade.C2S.CreateBuilder()
                            .SetPwdMD5(pwdMD5)
                            .SetSecurityFirm((int)securityFirm)
                            .SetUnlock(isUnlock)
                            .Build();
                ...
                Monitor.Wait(syncEvent);
                TrdUnlockTrade.Response resp = (TrdUnlockTrade.Response)reqInfo.Rsp;
                if (resp.RetType != (int)Common.RetType.RetType_Succeed)
                {
                    Console.WriteLine("ERROR: UnlockTrade, retMsg = {0}", resp.RetMsg);
                }
                return resp;
            }
        }
        ...
        public TrdSubAccPush.Response subTrdAccPushSync(List<ulong> accList)
        {
            ...
            lock (syncEvent)
            {
                ...
                Monitor.Wait(syncEvent);
                TrdSubAccPush.Response resp = (TrdSubAccPush.Response)reqInfo.Rsp;
                if (resp.RetType != (int)Common.RetType.RetType_Succeed)
                {
                    Console.WriteLine("ERROR: SubAccPush, retMsg = {0}", resp.RetMsg);
                }
                return resp;
            }
        }

        public TrdGetAccList.Response GetAccListSync()
        {
            ReqInfo reqInfo = null;
            Object syncEvent = new Object();
            ...
            lock (syncEvent)
            {
                lock (trdLock)
                {
                    if (trdConnStatus != ConnStatus.READY)
                        return null;
                    // SetUserID的参数由于历史原因，目前已废弃，参数填0即可
                    TrdGetAccList.C2S c2s = TrdGetAccList.C2S.CreateBuilder().SetUserID(0).Build();
                ...
                Monitor.Wait(syncEvent);
                TrdGetAccList.Response resp = (TrdGetAccList.Response)reqInfo.Rsp;
                if (resp.RetType != (int)Common.RetType.RetType_Succeed)
                {
                    Console.WriteLine("ERROR: GetAccList, retMsg = {0}", resp.RetMsg);
                }
                                          ...
                                          TrdCommon.Currency currency)
        {
            ...
            lock (syncEvent)
            {
                ...
                Monitor.Wait(syncEvent);
                TrdGetFunds.Response resp = (TrdGetFunds.Response)reqInfo.Rsp;
                if (resp.RetType != (int)Common.RetType.RetType_Succeed)
                {
                    Console.WriteLine("ERROR: GetFunds, retMsg = {0}", resp.RetMsg);
                }
                return resp;
            }
        }
                                              ...
                                              List<TrdCommon.OrderStatus> filterStatusList)
        {
            ...
            lock (syncEvent)
            {
                ...
                Monitor.Wait(syncEvent);
                TrdGetOrderList.Response resp = (TrdGetOrderList.Response)reqInfo.Rsp;
                if (resp.RetType != (int)Common.RetType.RetType_Succeed)
                {
                    Console.WriteLine("ERROR: GetOrderList, retMsg = {0}", resp.RetMsg);
                }
                return resp;
            }
        }
                                                          ...
                                                          TrdCommon.TrdFilterConditions filterConditions)
        {
            ...
            lock (syncEvent)
            {
                ...
                Monitor.Wait(syncEvent);
                TrdGetOrderFillList.Response resp = (TrdGetOrderFillList.Response)reqInfo.Rsp;
                if (resp.RetType != (int)Common.RetType.RetType_Succeed)
                {
                    // 模拟交易不支持成交数据，是正常情况
                    if (trdEnv == (int)TrdCommon.TrdEnv.TrdEnv_Simulate)
                    {
                        Console.WriteLine("Simulated trade does not support deal list");
                    }
                    else
                    {
                        Console.WriteLine("ERROR: GetOrderFillList, retMsg = {0}", resp.RetMsg);
                    }
                }
                return resp;
            }
                                                                ...
                                                                List<TrdCommon.OrderStatus> filterStatusList)
        {
            ...
            lock (syncEvent)
            {
                ...
                Monitor.Wait(syncEvent);
                TrdGetHistoryOrderList.Response resp = (TrdGetHistoryOrderList.Response)reqInfo.Rsp;
                if (resp.RetType != (int)Common.RetType.RetType_Succeed)
                {
                    Console.WriteLine("ERROR: GetHistoryOrderList, retMsg = {0}", resp.RetMsg);
                }
                return resp;
            }
        }
                                                                        ...
                                                                        TrdCommon.TrdFilterConditions filterConditions)
        {
            ...
            lock (syncEvent)
            {
                ...
                Monitor.Wait(syncEvent);
                TrdGetHistoryOrderFillList.Response resp = (TrdGetHistoryOrderFillList.Response)reqInfo.Rsp;
                if (resp.RetType != (int)Common.RetType.RetType_Succeed)
                {
                    Console.WriteLine("ERROR: GetHistoryOrderFillList, retMsg = {0}", resp.RetMsg);
                }
                return resp;
            }
        }
                                                        ...
                                                        bool isRefreshCache)
        {
            ...
            lock (syncEvent)
            {
                ...
                Monitor.Wait(syncEvent);
                TrdGetPositionList.Response resp = (TrdGetPositionList.Response)reqInfo.Rsp;
                if (resp.RetType != (int)Common.RetType.RetType_Succeed)
                {
                    Console.WriteLine("ERROR: GetPositionList, retMsg = {0}", resp.RetMsg);
                }
                return resp;
            }
